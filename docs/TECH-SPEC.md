...
### 5.4 Idempotency
- *Planned originally:* an `idempotencyKey` on every mutating Server Action, kept in an in-memory LRU. Never built.
- *Built (benchmark item 22, 2026-09-25):* the three field forms — customer update, new customer, close / reactivate — send a `submissionId` (a v4 UUID minted on the phone per payload) and submit over `fetch` to `POST /api/forms/[form]`, which calls the same actions. The id is stored on `CustomerEdit.submissionId`, unique per submitter. A submit whose id has already landed writes nothing and answers with a receipt (`replayed: true`, the request's state now): checked at the start of the submit, and again whenever the submit fails — an overlapping retry is refused by what its first attempt changed, and the receipt is then the true answer (`answerIfLanded` in `lib/submission-replay.ts`). The route checks the session itself and answers 401 when signed out; the middleware's sign-in gate does not stop it (see `auth.config.ts`). The new-customer form, whose branches, points and photos never live on the phone, also keeps the ids of sends that got no answer in its phone copy; after a reload it asks `GET /api/forms/customer-create?submissionId=` (the caller's own requests only) whether one landed, and opens that request instead of inviting a rebuild. The phone reuses an id only for the same payload after an attempt with no answer (`lib/submit-client.ts`). Approvals and other actions are not covered: they rely on atomic state claims instead.
