...
**What re-raises and what does not.** `cron.failed` is a *state* — a job that keeps failing fails again on its next run — so while it is still true it raises itself again when the next window opens, and a problem that lasts all day reaches you three or four times rather than once or twenty-four times. `sla.escalated` and `import.rejections` are *events*: each escalation is counted once, on the sweep that performs it, and each batch finishes once. One that falls inside an already-used window, or whose POST fails, is **not** sent later. So treat an SLA alert as "go and look", not as a complete list — the app's approvals queue is the list.
...
- **No retry.** A webhook that is down for the one POST loses that alert, and the window's single token has already been spent. A `cron.failed` that is still true re-raises after the window turns; an `import.rejections` or `sla.escalated` does not (see above).
