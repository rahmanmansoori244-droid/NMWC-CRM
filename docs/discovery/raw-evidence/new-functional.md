...
6. **Middleware** `middleware.ts` + `authorized()` callback `auth.config.ts:61-86` gate every non-public path; public = `/login`, `/api/auth`, `/api/health`, `/_next`, `/favicon.ico`. Also enforces **mustChangePassword** redirect to `/profile/change-password` `auth.config.ts:75-84`. Middleware also injects a per-request CSP nonce `middleware.ts:50-68`. [Confirmed]
