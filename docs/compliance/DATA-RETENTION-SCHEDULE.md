...
| Import payloads (`ImportRow.raw` / `parsed` / `issues`) | 90 days, then the verbatim spreadsheet copy is emptied; the row and its outcome are kept | `app/api/cron/retention-sweep/route.ts` (daily) | — |
