  ...
  sections: [
    ...
    },
    {
      ...
      steps: [
        ...
        {
          html: 'Review the <strong>QUARANTINED</strong> rows first — bad phone format, an unknown channel or visit-day code, a duplicate phone or CR on a different customer. When the reason names a customer already in the master (“phone already exists in master on customer X”), open that customer first: it may be the same shop. Each problem row has a <strong>Fix</strong> column: <strong>Correct…</strong> changes only the cells its problem names (never payment terms, credit or the Temix code) and checks the row again; <strong>Release shared phone…</strong>, offered only when the phone being on another customer is the row’s one problem, lets it through with your written reason; <strong>Re-check</strong> checks it again as it stands; <strong>Exclude…</strong> records, with a reason, that it stays out. A fixed row turns CLEAN, waits under <strong>Fixed, waiting to promote</strong>, and loads when you click <strong>Promote</strong>; until then <strong>Withdraw fix</strong> puts it back exactly as it was. The row as your sheet had it is kept; your corrections are shown beside it. For a customer linked to Temix a fixed row writes only its branch — a phone, name, CR, contact or channel you corrected or released there is <strong>not</strong> written to the customer (the row says so, before and after; change those on the customer’s page); a fixed row with no branch_code — every go-live head-office row — comes back REJECTED so you can give it one (a code the customer does not use yet adds a new branch). Withdrawing the fix of a rejected row takes its customer’s other rows back with it. Fix a row only in the newest upload that carries it: rows a newer upload superseded offer only Exclude, a fix that a newer upload overtakes while it waits is rejected at Promote, and rows uploaded more than 90 days ago can no longer be fixed. For a customer linked to Temix only a newer row about the same branch counts, so an inbound Temix refresh does not stop you fixing a held-back branch row.',
        },
        {
        ...
        {
          html: 'When it finishes, <strong>reconcile</strong>: <em>Left to promote</em> must be 0, and <em>Promoted + Rejected + Quarantined</em> must equal <em>Total</em>. Then open the <strong>Rejected</strong> view and read every row: a rejected row is <strong>not</strong> in the master. Typical reasons are a branch code that already belongs to another customer, a Temix code recorded on a different customer, or payment terms that disagree with the ones on record. Fix a rejected row as above — <strong>Re-check</strong> takes its whole customer with it — or exclude it. A payment-terms or crosswalk rejection comes back rejected until the master itself changes. Then open <strong>Loaded with a warning</strong>: those customers are in the master, but something in the row was substituted or not applied — a route or region that was not found, or, for a customer already linked to Temix, branch values in a file row, which a re-import never changes. Change those on the customer’s page. Note that the lane is decided by each ROW: a file row whose <strong>temix_code</strong> cell is blank takes the full lane even for a customer linked to Temix, overwriting its name, phone, CR, contact, channel, status and branch cells without telling Temix — so always fill temix_code with the customer’s own code.',
        },
        {
    ...
    },
    {
      ...
      intro: "Duplicates are the master's biggest enemy. The steward's queue surfaces two kinds: the same CR number, or the same name, phone and region.",
      steps: [
        {
      ...
      table: {
        ...
        rows: [
          ...
          [
            ...
            'Open customer X. If it is the same shop, exclude the held-back row. If it is a different shop sharing the owner’s phone, use Release shared phone… on the row, with the reason, then promote the batch. For a customer linked to Temix the release lets the row’s branch load but does not write the phone to the customer — add it on the customer’s page.',
          ],
          [
