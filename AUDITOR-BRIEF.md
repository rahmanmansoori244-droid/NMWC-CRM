...
**Written:** 2026-09-25. **Code described:** the head of branch `claude/nmwc-crm-consolidation-e10c1e` that carries this edit — `main` was `14a9356` (items 16 and 20, the third item-22 review) when it was written, and the post-merge fixes of those items sit on top of it until the owner merges. Check `git log origin/main` before comparing line numbers.
...
app/api/**           16 route handlers (below)
...
services/*.ts        15 'use server' modules exporting 47 async functions (server actions);
...
| `photos/attach`, `photos/detach` | POST | as `forms/[form]`: session checked in the route (401), JSON only (415), cross-site `Origin` refused (403); then `attachPhotoAction` / `detachPhotoAction` (`services/photos.ts`), whose role, uploader, kind and scope checks and zod validation decide |
...
- The duplicate detector and the create block now agree on EXACT_TRIPLE: any shared live-branch region, and one name comparison, `lib/name-key.ts` (item 16, owner decision 2026-09-25).
...
- **Submit gate** (`lib/submit-gate.ts`, UPDATE only): `CORE` by default (owner decision 2026-09-10) — channel, phone, contact, address ≥3 chars, GPS, shop photo. `SALESMAN_SUBMIT_GATE=FULL` adds sub-channel, CR number (CASH customers; locked on CREDIT) + CR photo, visit day, signboard. Salesmen only; drafts exempt. Separately, both field forms hold Submit — for every role — while any photo slot is still uploading, because the full-page load after a submit would cut the upload off (item 22, third review). The hold is bounded (`components/nmwc/PhotoCaptureSlot.tsx`): an upload silent for 45 s before its last byte has been counted out (retried twice), a wait for R2's answer after that longer than 45 s plus 1 s per 4 KB of photo (3¼ min for 600 KB; at most about 9¼ min, shorter than the presigned URL's 10-minute life; not retried — "too slow, move to better signal"), or a presign, finalize or attach unanswered for 30 s, ends in "Retry upload". A Remove unanswered for 30 s clears the slot anyway. Once a submit is on its way, no slot takes a new photo, retake, removal or Retry. CREATE has its own fixed rules (`lib/validation/create.ts`) that always require sub-channel, CR number and photo, visit day and signboard.

**Field submits over fetch (item 22, 2026-09-25)** — the three field forms (customer update, new customer, close/reactivate) submit by POSTing JSON to `app/api/forms/[form]/route.ts`, which calls the same server-action functions. Reason: a stalled server action cannot be aborted and every later action queues behind it (`lib/submit-client.ts` header). Since the post-merge review of `30ec23a` the photo slots' attach and Remove go the same way (`app/api/photos/attach`, `app/api/photos/detach`), and an attach of a photo to the slot it is already on is answered ok and writes nothing, so the slot's re-send of an attach that got no answer learns the truth. The update and new-customer forms' "Submitting…" state is their own flag, not `useTransition`: React settles pending transitions together, so any server action in flight on the page held it after the answer (close/reactivate, `components/nmwc/BranchStatusActions.tsx`, still uses `useTransition`).
...
- Hard duplicate blocks at submit (not for drafts): exact normalized CR, or EXACT_TRIPLE (legal name compared by `nameKey` — case-folded, whitespace runs collapsed — + normalized phone + any shared region), against live customers **and** other open CREATE requests (DRAFT, SUBMITTED, NEEDS_CORRECTION), serialized with transaction-scoped advisory locks. There is deliberately **no** unique index on CR (legacy duplicates); a stale comment in migration `20260510160000` says otherwise.
...
- The lane is decided **per row**. The **refresh lane** applies only when the stored `temixCode` **equals** the row's `temix_code` (a different code is rejected); it writes Temix code, payment terms and credit, and flips UPLOADED to SYNCED. A plain file row never creates or changes a branch (it is left PROMOTED with a `_lane` note). A row with a **blank** `temix_code` takes the ordinary lane even for a Temix-linked customer, overwriting CRM-owned customer fields and branch cells without re-queuing for Temix (documented in OPERATIONS §7; not changed). A row the Steward **fixed in the app**, for a Temix-linked customer, is always "branch only" (owner decision 2026-09-25), whatever its `temix_code` cell and whatever rows are beside it — decided **per row** since the post-merge review (it was per group, so a fixed row beside a plain one took the full lane): it creates its branch or updates it from the cells it gave, writes nothing about the customer (not name, phone, CR, contact, channel, nor the credit figures), re-queues the customer as PENDING_UPLOAD, and alone does not flip UPLOADED to SYNCED; the plain rows beside it take the lane their first row decides. A customer-level value the Steward corrected or released on such a row is not written, and a `_lane` note on the promoted row says so (`unwrittenCustomerCells` / `branchOnlyNote` in `lib/import-row-fix.ts`); the batch page warns before a release or a customer-level correction. A fixed row that cannot be applied (no `branch_code`, another customer's code, an archived branch) throws a CROSSWALK error so the customer is REJECTED with `branch_code` editable.
- On the ordinary lane (item 20, owner decisions): a customer's status follows all its live branches — settled after the branch writes, a blank status cell saying nothing — not only the file's rows; a blank branch cell keeps the stored value on update; a row with no `branch_code` is rejected once the customer has branches.
- The per-row upload check is `lib/import-row-check.ts`, shared by the upload and the Steward's in-app fix (`services/import-fixes.ts`): **re-check** (a rejected row takes its whole customer), **correct** only the cells a problem names (never payment terms, credit or the Temix code; the audit row names columns, not values), **release** a row held back only for a phone already on another customer (reason, FORCE_OVERRIDE), **exclude** / include again, and **withdraw** a fix still waiting to promote (back to the row's pre-fix state and reasons, kept in `parsed.fixedFrom`). Every action locks the batch row `FOR UPDATE` and refuses while a promote holds a live lease. Only the row acted on is marked `fixedInApp`; a rejected row's siblings come back CLEAN as plain rows, and **withdraw** takes them back with it (they carry `fixedFrom`) and sets a promoted batch the fix had set back to READY to PROMOTED again. Re-check, correct and release also refuse in an upload older than another that supersedes the row — any newer upload of a customer not linked to Temix; for a linked customer only a newer row about the same branch that wrote or may write it, never a plain refresh row (`supersedingUpload` in `lib/import-row-fix.ts`, fed by `newerUploadsCarrying` in `lib/import-master-lookup.ts`, which the page, the fix actions and promote all use) — and on a row uploaded more than 90 days ago (`IMPORT_PAYLOAD_DAYS`, shared with the retention sweep, after which the newer uploads' data may be gone). Promote asks the newest-upload question again for fixed rows and rejects one a newer upload overtook while it waited. The page offers only Exclude on a superseded or out-of-window row, in the server's words. Exclude and include deliberately do neither, so an old batch can always leave Work. Only changed cells are recorded. `ImportRow` gained `corrections` (raw stays as uploaded) and `excludedAt/By/Reason` (migration `20260926090000_import_row_fixes`).
- The batch page (`app/(app)/import/[batchId]`, helpers in `lib/import-rows-view.ts`) pages through every row of seven views (needs attention, fixed waiting to promote, rejected, quarantined, loaded with a warning, excluded, all), shows each row as uploaded with its corrections, and carries the fix actions. The Work list shows a customer batch while it has a problem row neither fixed nor excluded, or fixed rows waiting to be promoted.
...
**Duplicates & merge** (`services/duplicates.ts`; the pairing rules and the pair count are the pure `lib/duplicate-pairing.ts`): exact CR or EXACT_TRIPLE only (phone-only and fuzzy matching were dropped); candidates are computed when the Duplicates page opens, which shows the true pair count and the first 50. Dismissals ("Mark distinct", which asks first) are stored as `AuditLog` rows (`entityType 'CustomerPair'`) and read back as state — business decisions in the append-only ledger. Since item 16 a dismissal stores digests of what the pair shared (`after.signals`: truncated sha256 of the CR norm and of name key + phone — unkeyed, so a 7-digit CR can be recovered by brute force) and lapses when the pair comes to match on something new; an undo is a later `after: { undo: true }` row, and the latest row per pair wins. Legacy rows with no `after` still hide their pair for good. Merge locks both rows, auto-rejects the loser's open edit (the only place `CustomerEdit.state = REJECTED` is written), moves branches/edits/attachments to the winner, soft-deletes the loser, and queues its Temix deactivation if Temix knew it.
...
- **CI** (`.github/workflows/ci.yml`, every push to every branch; superseded runs cancelled except on `main`):
  ...
  - `db-tests`: migrate, seed, `app-role.ts` create/grant/verify, generated fixtures, the PII-inventory check, and 26 of the 29 integration suites (17 `RUN_*` flags; `build-chain-data`, `golive-rehearsal` and `uat-load` are not run). It connects as the owner role; only `audit-immutability` connects as `nmwc_app`.
...
| Rule | Status in code |
|---|---|
| Never write to the production DB (`ep-sweet-haze`); "every gated test asserts" it | **23 of 29** integration suites throw on `DATABASE_URL` in `beforeAll`. The other **6 do not**: `import-reconciliation`, `import-region-codes` and `rate-limit-pg` have no production check at all; `merge-attachment-reparent`, `merge-concurrency` and `reactivation-authz` call the purge helper, which checks only `DIRECT_URL` — that stops the two merge suites before their first write, but runs after the writes in `reactivation-authz`. There is no central guard. `app-role.ts`, `restore-verify.ts`, `prisma/synthetic.ts` and `seed-muscat-pilot.ts` refuse production **unless** `ALLOW_PRODUCTION=1` / `ALLOW_PROD_SEED=1` is set ("outright" in CLAUDE.md is too strong). `scripts/qa/run-with-env.mjs` loads `.env` and has no host check. |
...
- Before any push: `npm run typecheck` (runs `next typegen` first — bare `tsc` misses route types), `npm run lint`, `npm test`. Integration suites run locally against the UAT branch by practice (`scripts/qa/run-with-env.mjs` just loads `.env`); CI runs 26 of them.
...
- Commit messages are the design record for recent work: `git log --format=%B` for `54de6aa`, `666bce8`, `e7db028`, `b239e4b`, `3e775d4`, `efe3729`, `b7d9041`, `ab5867f`, `2d1e702`; for items 16 and 20 and the third item-22 review, `84c5b74`, `8290342`, `e5d4043`, `215bf41`, `374b0e7`, `748a164`, `306a0d8`, and the fixes of their pre-merge review, `4482c4c`, `30ec23a`, `14a9356`.
...
- Item 16: `normalizeCR` now folds Arabic-Indic/Persian digits and strips invisible characters, but the stored `crNumberNorm` of existing rows is recomputed only when `scripts/ops/recompute-cr-norm.ts` runs against production — the owner's go-ahead is pending, and until then those twins still do not match. The run keeps a pair the Steward marked distinct on a CR the two customers already shared (its digest changes, its match does not) by appending a carried-forward `CustomerPair` row; the dry run counts them. Placeholder CRs such as `0` can still fill the page; names differing only by a zero-width character still differ; the dismissal digests are unkeyed; the full scan still runs on every page open (item 15). Between the deploy and the production recompute, a CR typed with Arabic-Indic digits or an invisible character also stops matching an identical stored one (the incoming side folds, the stored side does not) — the recompute's read-only dry run measures that exposure.
- Item 20: a file re-import still cannot update an existing branch of a Temix-linked customer (by design: only an in-app fix can); a file row with a blank `temix_code` still takes the full lane for a Temix-linked customer; a phone released or corrected on a fixed row of a Temix-linked customer is not written even when the customer has no phone (branch only; the row says so — whether an empty phone should be filled is an owner question); account-master rows have no in-app fix (re-upload); the payload of a rejected row is still swept after 90 days, after which it can only be excluded or uploaded again; a held-back row never fixed or excluded keeps its payload (DATA-RETENTION-SCHEDULE gap 6); there is no download of problem rows.
- Item 22 (third review): the photo hold is bounded, not short — an upload that goes silent before its last byte takes about 2¼ minutes to give up after its retries, and one whose last byte has been counted out waits up to 45 s plus 1 s per 4 KB of photo for R2 (about 9¼ minutes at most, not retried); a busy slot has no Cancel; an attach that lands after its 30 s wait is discovered only through Retry (which re-sends the attach alone and is answered ok) or a reload; a late attach can still land after a retake and replace the retaken photo; a Remove the server refused still clears the slot on the phone; after "Save draft" on a new customer, Back from Work shows the empty form (`history.replaceState`), and a create draft cannot be discarded (owner decision pending).
...
- **EXACT_TRIPLE** — duplicate key: legal name by `nameKey` (case-folded, whitespace collapsed) + normalized phone + any shared live-branch region.
...
**Weeks (9–32)**
9 SLOs/dashboard · 10 log search/tracing · 11 health goes red for minor things · 12 unconfigured R2 reads healthy · 13 one person holds every credential · 14 app connects as the DB owner · 15 duplicates found only when the screen opens · **16 no tests on duplicate detection — done 2026-09-26 (tests, then the owner's rule decisions: any region, spacing, Arabic digits, lapsing dismissals with undo; production CR recompute awaiting the owner)** · 17 merge not undoable and discards better values · 18 no data-quality history · 19 quality score counts "Address pending" as data · **20 Steward cannot fix quarantined/rejected import rows in-app — done 2026-09-26 (re-check, correct, release shared phone, exclude; branch-only lane for Temix-linked customers; three re-import traps closed)** · 21 not installable/offline · **22 lost connection at submit — done** (`b7d9041`, `ab5867f`, `2d1e702`) · 23 photos cannot be taken offline · 24 nothing measures plan adherence · 25 Today covers about a third of the route · 26 Arabic only in stale PDFs · 27 notifications in-app only · **28 Excel export capped at 25,000 — done** (`b239e4b`, `3e775d4`, `efe3729`) · 29 no API · 30 no machine accounts · 31 no feature flags/staging · 32 three test suites never run in CI.
