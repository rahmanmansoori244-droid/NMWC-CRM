...
tests/unit           80 files, 1,211 tests (vitest + jsdom); one runs only where golive-data/ exists
...
- **Submit gate** (`lib/submit-gate.ts`, UPDATE only): `CORE` by default (owner decision 2026-09-10) — channel, phone, contact, address ≥3 chars, GPS, shop photo. `SALESMAN_SUBMIT_GATE=FULL` adds sub-channel, CR number (CASH customers; locked on CREDIT) + CR photo, visit day, signboard. Salesmen only; drafts exempt. Separately, both field forms hold Submit — for every role — while any photo slot is still uploading, because the full-page load after a submit would cut the upload off (item 22, third review). The hold is bounded: an upload silent for 45 s, or a presign, finalize or attach unanswered for 30 s, ends in "Retry upload" (`components/nmwc/PhotoCaptureSlot.tsx`). Once a submit is on its way, no slot takes a new photo, retake, removal or Retry. CREATE has its own fixed rules (`lib/validation/create.ts`) that always require sub-channel, CR number and photo, visit day and signboard.
...
- **What the tests actually exercise (read "1,211 unit tests" with this in mind):**
...
npm run lint
npm test                 # 1,211 unit tests (one skips without golive-data/); integration files collect and skip
...
- Item 22 (third review): the photo hold is bounded, not short — a dead upload takes about 2¼ minutes to give up after its retries, and a busy slot has no Cancel; an attach that lands after its 30 s wait is discovered only through Retry or a reload; after "Save draft" on a new customer, Back from Work shows the empty form (`history.replaceState`), and a create draft cannot be discarded (owner decision pending).
