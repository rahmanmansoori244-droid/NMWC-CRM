...
- The lane is decided **per row**. The **refresh lane** applies only when the stored `temixCode` **equals** the row's `temix_code` (a different code is rejected); it writes Temix code, payment terms and credit, and flips UPLOADED to SYNCED. A plain file row never creates or changes a branch (it is left PROMOTED with a `_lane` note). A row with a **blank** `temix_code` takes the ordinary lane even for a Temix-linked customer, overwriting CRM-owned customer fields and branch cells without re-queuing for Temix (documented in OPERATIONS §7; not changed). A row the Steward **fixed in the app**, for a Temix-linked customer, is always "branch only" (owner decision 2026-09-25), whatever its `temix_code` cell: it creates its branch or updates it from the cells it gave, re-queues the customer as PENDING_UPLOAD, and does not flip UPLOADED to SYNCED; a fixed row that cannot be applied (no `branch_code`, another customer's code, an archived branch) throws a CROSSWALK error so the customer is REJECTED with `branch_code` editable.
- On the ordinary lane (item 20, owner decisions): a customer's status follows all its live branches, not only the file's rows; a blank branch cell keeps the stored value on update; a row with no `branch_code` is rejected once the customer has branches.
- The per-row upload check is `lib/import-row-check.ts`, shared by the upload and the Steward's in-app fix (`services/import-fixes.ts`): **re-check** (a rejected row takes its whole customer), **correct** only the cells a problem names (never payment terms, credit or the Temix code; the audit row names columns, not values), **release** a row held back only for a phone already on another customer (reason, FORCE_OVERRIDE), **exclude** / include again, and **withdraw** a fix still waiting to promote (back to the row's pre-fix state and reasons, kept in `parsed.fixedFrom`). Every action locks the batch row `FOR UPDATE` and refuses while a promote holds a live lease. Re-check, correct and release also refuse in an upload older than another carrying the same customer (worded by what that newer row is; the page offers only Exclude on such rows — `newerUploadsCarrying` in `lib/import-master-lookup.ts` serves both) and on a row uploaded more than 90 days ago (`IMPORT_PAYLOAD_DAYS`, shared with the retention sweep, after which the newer uploads' data may be gone). Exclude and include deliberately do neither, so an old batch can always leave Work. Only changed cells are recorded. `ImportRow` gained `corrections` (raw stays as uploaded) and `excludedAt/By/Reason` (migration `20260926090000_import_row_fixes`).
...
- Commit messages are the design record for recent work: `git log --format=%B` for `54de6aa`, `666bce8`, `e7db028`, `b239e4b`, `3e775d4`, `efe3729`, `b7d9041`, `ab5867f`, `2d1e702`; for items 16 and 20 and the third item-22 review, `84c5b74`, `8290342`, `e5d4043`, `215bf41`, `374b0e7`, `748a164`, `306a0d8`.
