...
tests/unit           78 files, 1,179 tests (vitest + jsdom); one runs only where golive-data/ exists
...
- **Submit gate** (`lib/submit-gate.ts`, UPDATE only): `CORE` by default (owner decision 2026-09-10) — channel, phone, contact, address ≥3 chars, GPS, shop photo. `SALESMAN_SUBMIT_GATE=FULL` adds sub-channel, CR number (CASH customers; locked on CREDIT) + CR photo, visit day, signboard. Salesmen only; drafts exempt. Separately, both field forms hold Submit — for every role — while any photo slot is still uploading, because the full-page load after a submit would cut the upload off (item 22, third review). CREATE has its own fixed rules (`lib/validation/create.ts`) that always require sub-channel, CR number and photo, visit day and signboard.
...
- **What the tests actually exercise (read "1,179 unit tests" with this in mind):**
...
npm run lint
npm test                 # 1,179 unit tests (one skips without golive-data/); integration files collect and skip
...
- Item 22 (third review): a photo started while a Submit is already on its way can still be cut off by the page load that follows the answer; after "Save draft" on a new customer, Back from Work shows the empty form (`history.replaceState`), and a create draft cannot be discarded (owner decision pending).
