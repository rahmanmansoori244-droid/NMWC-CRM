...
- The **refresh lane** applies only when the stored `temixCode` **equals** the row's `temix_code` (a different code is rejected); it writes Temix code, payment terms and credit, and flips UPLOADED to SYNCED. A plain file row never creates or changes a branch (it is left PROMOTED with a `_lane` note); a row the Steward **fixed in the app** creates its branch or updates it from the cells it gave, never moves one from another customer, and re-queues the customer as PENDING_UPLOAD (owner decision 2026-09-25, "branch only").
