...
- Some code still assumes one branch per customer (the May "flatten"): the duplicate detector (`lib/duplicate-pairing.ts`) keys EXACT_TRIPLE on the region of the customer's first live branch by branch code, while the create guard uses any live branch — an open owner question (item 16), pinned by tests as today's behaviour.
...
- The **refresh lane** applies only when the stored `temixCode` **equals** the row's `temix_code` (a different code is rejected); it writes Temix code, payment terms and credit, and flips UPLOADED to SYNCED. It never creates or changes a branch; a row whose branch is missing or differs is left PROMOTED with a `_lane` warning (item 20).
- The batch page (`app/(app)/import/[batchId]`, helpers in `lib/import-rows-view.ts`) pages through every row of five views (needs attention, rejected, quarantined, loaded with a warning, all) and shows each row as uploaded. The Steward still cannot fix, retry, release or dismiss a row in the app (item 20's owner decisions are open); the fix path is a corrected re-upload, with the hazards listed in `docs/OPERATIONS.md` §7 "Re-import the master".
...
- Item 16: what the detector should match is open with the owner — multi-branch region, inner whitespace in names, Arabic-Indic digits and zero-width characters in CRs (tests hold these as `it.todo` or as today's answer), placeholder CRs such as `0` filling the page, and whether a dismissal should lapse or be undoable. The full scan still runs on every page open (item 15).
- Item 20: no in-app fix, retry, release or "accept as excluded" for a problem row; the Work list still shows a batch for as long as its upload-time quarantine count is above zero and never for rejections alone; a full-lane re-import can close a customer from a partial file, overwrite branch fields with placeholders, and (without `branch_code`) overwrite a sibling branch.
...
**Weeks (9–32)**
9 SLOs/dashboard · 10 log search/tracing · 11 health goes red for minor things · 12 unconfigured R2 reads healthy · 13 one person holds every credential · 14 app connects as the DB owner · 15 duplicates found only when the screen opens · **16 no tests on duplicate detection — tests added 2026-09-25; what the rules should match is with the owner** · 17 merge not undoable and discards better values · 18 no data-quality history · 19 quality score counts "Address pending" as data · **20 Steward cannot fix quarantined/rejected import rows in-app — every row now reachable and readable, and warnings added, 2026-09-25; the fix actions wait on owner decisions** · 21 not installable/offline · **22 lost connection at submit — done** (`b7d9041`, `ab5867f`, `2d1e702`) · 23 photos cannot be taken offline · 24 nothing measures plan adherence · 25 Today covers about a third of the route · 26 Arabic only in stale PDFs · 27 notifications in-app only · **28 Excel export capped at 25,000 — done** (`b239e4b`, `3e775d4`, `efe3729`) · 29 no API · 30 no machine accounts · 31 no feature flags/staging · 32 three test suites never run in CI.
