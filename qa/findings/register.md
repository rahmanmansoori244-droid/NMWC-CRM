...
- **C1 — CONFIRMED (inert gate, no exposure). Corrected 2026-09-15.** This entry previously read "REFUTED (no defect)" and listed the Auth.js middleware `authorized` callback as layer (1) of five. That was wrong, and the original premise it dismissed was right. In next-auth `5.0.0-beta.31`, `handleAuth` (`node_modules/next-auth/lib/index.js`) tests `authorized instanceof Response`, then `userMiddlewareOrRoute`, then `!authorized` — and because `middleware.ts` wraps a function (the CSP-nonce middleware added for B-13), `userMiddlewareOrRoute` is always truthy, so the `!authorized` branch is unreachable and the `false` returned by the callback is discarded. The same callback's `Response.redirect` for `mustChangePassword` hits the first branch and *is* honoured, which is why AUTH-09 works and why this went unnoticed: one line of the callback fires and the line above it does not, purely by return type. **There is no live exposure** — the layers that actually hold are the ones the original premise named: `app/(app)/layout.tsx` calls `auth()` and redirects, every page and route handler re-checks, and server actions call `requireSession()`. The verdict is corrected rather than the code, deliberately: returning a `Response` from the deny path would redirect RSC fetches and Server Action POSTs, which is the exact failure recorded at `docs/SESSION-MASTER-RECORD.md` §143(d) — a middleware redirect on a Server Action POST produced "An unexpected response was received from the server" and left users unable to sign in. Note also that this entry's original citation, `auth.config.ts:69-70`, never pointed at the gate; the gate is the `if (!auth) return false` line inside `authorized`. `docs/OPUS-QA-EXECUTION-PLAN.md:86` identified the branch-order cause correctly and was overruled.
