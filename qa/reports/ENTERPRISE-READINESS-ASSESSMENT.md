...
| — | **B4** AuditLog immutability + least-privilege DB role | Open | Migration (trigger) + `nmwc_app` role + Vercel `DATABASE_URL` swap; Neon role capability Not Verified. |
| — | **B5** sub-daily scheduling + heartbeat | Open | Decision D3 (Vercel Pro crons vs external scheduler) is the owner's; heartbeat + health 503 can be built now. |
