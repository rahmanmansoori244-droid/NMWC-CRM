...
| — | **B3** restore drill / RTO-RPO | Open | Needs `NEON_API_KEY` + `NEON_PROJECT_ID` GitHub secrets (owner) before the drill can run. |
...
| 2026-09-14 | **B5** sub-daily scheduling + heartbeat | **Dead-man built — scheduler decision D3 pending (owner)** | `CronHeartbeat` table (migration `20260914150100_cron_heartbeat`) + `lib/heartbeat.ts`: `withHeartbeat` wraps `sla-escalate` / `keep-warm` / `photo-gc` so every finished run is recorded (401 excluded; a thrown handler records a failure and answers 500); `heartbeatReport` states `ok / failed / stale / never / outside-window` against the schedules (30 min and 4 min inside 03–15 UTC, daily), stale after 3 missed intervals. `/api/health`: anonymous **503** when `SELECT 1` fails (was always 200); bearer mode adds `cron.alarms` + `cron.jobs` and answers **503** on any alarm — the URL for an external monitor. keep-warm 503 on DB failure. Unit `tests/unit/heartbeat.test.ts` 6/6. The scheduler itself is still undelivered: OPERATIONS §5d gives both options (Vercel Pro `crons` vs external caller) with exact endpoints and cadences; until D3 is decided the probe says `stale`/`never` — which is the point. |
...
| — | **B6** residency / PDPL register | Open | Owner + legal decision; register template can be drafted. |
