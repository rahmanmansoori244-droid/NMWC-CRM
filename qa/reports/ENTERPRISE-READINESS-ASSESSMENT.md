...
own path. The twenty-two below are real and were NOT fixed: none of them blocks
...
| P2 | app-role-grants | restore-verify --expect-app-role documents that it checks the role's REVOKEs but only checks that the role name exists | `restore-verify.ts:487` | The script prints "✓ restore verified" and exits 0 in both failure directions. If grant was skipped, nmwc_app has no privileges at all on the restored database — pg_dump --no-privileges strips every GRANT — so pointing Vercel at the restored database gives "permission denied for table Customer" on literally every page, |
...
| P3 | golive-passwords-and-bootstrap | account-master.xlsx holds every initial password in cleartext, but the runbook's clean-up step names only credentials.xlsx and managers.json | `GO-LIVE-RUNBOOK.md:121` | golive-data/account-master.xlsx stays on the laptop with a `password` column holding the live 8-digit secret for every salesman and for `accountant`, `finance.manager` and `gm.nmwc`. Those values are dead only for people who have actually signed in — and the runbook itself (§1 step 8) tells the Steward to check the nex |
| P3 | golive-passwords-and-bootstrap | RECONCILIATION.md tells the owner Sara Khayat's second login is `sara.khayat.c3`; the builder actually issues `c3` | `build-masters.ts:1282` | There is no `sara.khayat.c3` row anywhere — the second, salesman login is `c3`. The runbook (§1 step 8) warns that a manager who also sells has two logins and that "they will try the wrong one"; the reconciliation document the owner is told to read after every build names a username that does not exist, which is exactl |
...
| P3 | audit-immutability | `restore-verify --expect-app-role` claims to check the REVOKEs and only checks that a role named nmwc_app exists | `restore-verify.ts:487` | The operator's closing green says `R-01 least-privilege role nmwc_app exists — present ✓` and they conclude the restored database is back to the documented privilege posture. It is not: the role can hold every privilege on both ledgers, or none at all, and R-01 reports `pass` either way. The one check that would catch  |
