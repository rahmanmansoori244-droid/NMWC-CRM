...
Seven closed in a first pass; the four left over were worked in a second and are
now closed too, except for three carve-outs that are named in their own rows
rather than quietly marked done: whether a failed audit write should fail the
user's action, whether archiving a customer releases the documents behind its
credit decision, and the Steward's org-wide administrative reach.

Two of that last four turned out to be hiding defects worse than the row. The
merge path aborted outright for every duplicate pair where only the loser held a
commercial-registration document, and the go-live builder was about to create all
57 accounts with one shared password.
...
| DG-03/04 | **Half closed; the other half stays an owner decision.** The retention sweep landed in the first pass. Designing the archive half surfaced two live defects on the merge path instead: `Customer.crPhotoId` is backed by a non-deferrable unique index, so setting the winner's slot while the loser still held the same id aborted the whole merge with an opaque P2002 — every pair where only the loser had a CR document — and moving the pointer never moved `Attachment.customerId`, so the winner's CR and GUARANTEE documents 404'd for every role but Steward and Viewer, and the guarantee count in the Temix export understated the evidence behind the winner's own credit limit. Both fixed; three tests, two of them red against the pre-fix code. Merging is now settled policy — the loser's documents are re-parented, never released, because a merge asserts the two rows are the same legal entity. Two questions stay open, both needing a period or a policy from the owner rather than code: whether archiving a customer should release those documents, and how long a change request may sit in `DRAFT` or `NEEDS_CORRECTION` before the photographs claimed against it are swept. Both are gaps 1 and 5 in the retention schedule. |
| DG-06/07 | **Closed, with one carve-out named.** All ~60 direct writers across thirteen files now go through `writeAudit`, so `ip` and `userAgent` are populated everywhere; the SLA sweep uses an explicit system envelope rather than stamping the scheduler's address onto a human's row; the mislabelled export rows were fixed with B6. A lint rule and `tests/unit/audit-guard.test.ts` stop the next direct write from landing — and `services/` is now actually linted, having been linted by nothing at all, which is where 23 of the writes lived. The rule stops at the request-serving tree: five operator scripts write the ledger by hand with no request behind them, and that class is documented rather than converted. Not done: whether a failed audit write should fail the user's action. Two best-effort sites still swallow it; the other 34 sit inside the caller's transaction and already abort with it. |
| SEC-11 | **Closed in code, and the distribution problem was the real one.** The go-live builder was creating all 57 accounts — Steward, 11 managers, 42 salesmen, 3 approvers — with one shared constant, `12345`. Usernames are route codes printed on the journey plan, so the exposure was attribution, not brute force: anyone who heard the password could sign in as a colleague who had not signed in yet and own their audit trail from then on. Each account now draws its own 8-digit value from one issuer per run: digits only because it is read off paper and typed on a phone, no leading zero, weak runs redrawn, no environment override. `mustChangePassword` still pins the account to the change-password screen, so the value works exactly once. The runbook's step 1 and step 8 were contradicting each other four paragraphs apart and now describe one procedure: one row per person, never the sheet to a room. Not done: purging the old shared credentials from git history. |
| SEC-03/09, DG-09 | **Two of the four claims closed.** An ordinary spreadsheet could create a live CREDIT customer at any limit, or flip an existing customer's terms, with no approver and nothing in the trail — while the same change made in the app runs Salesman → Supervisor → Finance Manager → GM → Accountant, and an ordinary edit cannot touch payment terms at all. Two guards now hold those rows on the non-refresh lane only, in both directions, since a silent CREDIT→CASH flip leaves a live credit limit that the Temix export then suppresses. A test asserts the go-live master still loads, because every row the builder emits carries `temix_code` and would otherwise have been blocked at launch. Direct writes now record a reason naming the path and the role held at the time. Not done: the Steward's org-wide administrative reach, and a two-person rule for provisioning approver-tier accounts. Both are role-design questions, not defects. |
...
**Status, 2026-09-15.** All eleven claims were re-verified against the code as it
stands, one reader each, and every resulting plan was challenged by a second
reader before a line was written. Every one of the seven plans came back needing
revision, and three of those objections were defects the plan would have shipped.

Three claims turned out to be worse than the row that named them, and one of those
is the reason this pass was worth running at all.

| Claim | Outcome |
|---|---|
| SEC-14e — uploads not content-sniffed | **Worse than stated, and closed.** The MIME allowlist on the presign route cannot bind anything: the S3 request presigner marks `content-type` unsignable by design, so it never reaches `SignedHeaders` and a client may PUT any type it likes. `finalize` copied that into `Attachment.mimeType` and the serving route echoed it back, from the app's own origin. A salesman could store an HTML document, attach it to a slot on his own route, and the supervisor or GM who opened the tile — which the role guide tells them to do — got an attacker-written page on the domain they had just signed into. Not stored XSS, because the nonce policy carries `strict-dynamic`; but with no `form-action` a fake "session expired" form could post an approver's credentials anywhere. `lib/photo-mime.ts` now pins the served type. |
| SEC-14b — CSP lacks `base-uri` / `form-action` | **Closed.** Both absent from both policies, and neither falls back to `default-src`, so every page could post a form to any host. No live exposure — there is no HTML-injection sink in the tree — but `form-action` is the second layer under SEC-14e. The two hand-duplicated policies now come from one builder, which the remediation record asked for eighteen weeks ago, and a test re-implements Next's nonce extractor so the blank-production failure mode is pinned. |
| SEC-14d — edge Sentry unscrubbed | **The claim was already fixed; verifying it found something worse.** The scrubber ran on `beforeSend` only, and Sentry routes performance transactions to a different hook that nothing set — so roughly one request in ten shipped its full URL, customer search term included, to a processor outside Oman, in all three runtimes. The error channel leaked the same field on 100% of events through navigation and fetch breadcrumbs. The pattern also claimed to match Omani mobiles "in any written form" and matched almost none of them, including the spaced form in this repo's own go-live fixture. |
| DO-16/17, UAT-17/18 — stale docs / env contract | **Partly true, and the true part was reachable this week.** The blanket claim is out of date. But `scripts/print-required-secrets.ts` — which both OPERATIONS.md and the runbook call "the checklist" — had not been touched since May and did not name `BACKUP_AGE_RECIPIENTS`. An owner following it would have had a nightly plaintext copy of the customer master and every password hash in object storage. The list is now generated and drift-tested, and the backup refuses to upload plaintext instead of warning about it. |
| SEC-14c — `__Host-` cookie | **Closed, on timing rather than threat.** Nothing is exposed today: vercel.app is a public suffix. The hole opens automatically at the custom-domain cutover, and after launch the same edit costs every user their session. Also found alongside: the maintenance bypass cookie was compared with `===`, an unlisted instance of the very thing SEC-14 flagged elsewhere. |
| UAT-15 — segment loading/error boundaries | **Half already closed, half fixed, one part deliberately not.** Loading is well covered: 17 of 18 segments ship a real skeleton. Error coverage was the gap, and a throw took the whole authenticated shell with it. The hard-load blank screen is left alone on purpose — repainting sooner would demote every server redirect in the tree, including the one that sends unauthenticated users to the login page. |
| UAT-07/08 — a11y / i18n | **Claim wrong as written; the narrow part fixed.** There is deliberate, commented accessibility work throughout. The real gap was 44 orphaned labels on the two forms salesmen use all day, and a reject dialog with no focus trap or Escape. Arabic is correctly a roadmap item. |
| SEC-14a — health `===` compare | **Already fixed**, in the deployed production commit. Every one of the six secret comparisons in the repo shares one constant-time helper. It had no test, which is the part that was worth doing. |
| DG-14 — Temix batch persistence | **Never true.** `TemixSyncBatch` has existed since the Phase 1a schema. |
| DG-16 — channel CRUD | **Real, and correctly not now.** One seed script is the only writer of a Channel row. The go-live load is unaffected: every value the master builder emits is one of the seven seeded keys, and an unknown code quarantines the row with a readable reason. The cost lands post-launch, when a Steward needs an eighth channel. |
| SCALE-17 — per-lambda caches | **Partly true, no wrong answer.** Two module-level caches hold data, both deliberate and TTL-bounded. The authorisation one makes revocation roughly ten times faster than the five-minute staleness the JWT design already accepts, so it tightens the contract rather than loosening it. |
...
|---|---|---|---|
| 2026-09-15 | **P3 list** — eleven claims, seven of them real | **Closed, with the browser walk still owed** | The one that mattered: the presigned upload never bound Content-Type (the S3 presigner marks that header unsignable), so a salesman could store an HTML document and the photo route served it from this origin to the approver who opened the tile. `lib/photo-mime.ts` pins the served type; 18 tests. Alongside it: `form-action`, `base-uri` and `object-src` added to both policies, now built by one `lib/csp.ts` with a test that re-implements Next’s nonce extractor; Sentry transactions and breadcrumb URLs scrubbed (they were shipping the customer search term to a processor outside Oman, 10% and 100% of events respectively) and the phone pattern widened to the forms `lib/phone.ts` actually accepts; the secrets checklist generated from `lib/ops/required-secrets.ts` and drift-tested against the workflows, with the nightly backup now REFUSING a plaintext upload rather than warning; `__Host-` on the session cookie while it is still free, plus a constant-time compare for the maintenance bypass; an (app)-level error boundary so a failed page no longer takes the navigation with it; 44 orphaned form labels associated via one shared component, and a focus trap on the reject dialog. Two claims were already fixed (SEC-14a, edge Sentry), one was never true (DG-14), and two are real but correctly post-launch (DG-16, SCALE-17). Commits `3edff8d`, `5ecda53`, `86adae0`, `0288b5d`, `a7ae6b7`, `5d0a23b`. **Owed:** the real-browser pass on a Preview for the CSP and the form changes. |
...
| 2026-09-15 | **B3/B6 deployed; B4 role live on production** | **Merged** | Pre-flight review before the merge found two P0s (the retention sweep would have made a staged import unpromotable; the restore drill's drop guard was inert because its repository variable did not exist) — both fixed with tests before the fast-forward. The new backup workflow was proven against real production *from the branch* before the merge, because GitHub runs schedules from the default branch. `main` = `3f753ef`; production verified (health 200, login 200, cron routes 401, region iad1, `backup-report → HTTP 200`). The least-privilege role `nmwc_app` now **exists on production**, provisioned by a dispatch-only workflow that keeps the owner credential inside Actions, with every refusal proven connected as the role. **The application still connects as the owner** — the Vercel `DATABASE_URL` swap is the remaining step and is the owner's. Still open and now stated honestly in the documents: backups are plaintext until the age key exists, and the monthly drill fails red until the Neon secrets are added. |
...
## Open backlog from the blocker/P2 hostile pass (2026-09-15)

Seven hostile readers over the B1–B6 and P2 code, which had shipped reviewed for
correctness by its own author and had never had an adversarial reader. Thirty-five
findings. Thirteen were fixed and deployed the same day — two that would each have
stopped or silently spoiled the go-live load, five P1s, and six on the operator's
own path. The twenty-two below were real and NOT fixed when this was written. **Four have since been closed and are struck through in place** — three of them incidentally, while doing other work, which is the honest reason to re-read a backlog rather than trust its header. Eighteen remain open: none of them blocks
the load, and shipping a long tail of changes into the week of a launch is its own
risk. They are recorded here rather than in a conversation so they survive.

Read the severity as "consequence if it happens", not "likelihood". Several of
these are one-line fixes whose only cost is that they change behaviour nobody has
re-tested this close to launch.

| Sev | Subsystem | Finding | Where | Consequence |
|---|---|---|---|---|
| P2 | heartbeat-dead-man | `failed` alarms are sticky and window-blind, so one transient blip on the last keep-warm run of the day holds /api/health at 503 for ~12 overnight hours | `heartbeat.ts:203` | From 14:56 to ~03:04 UTC — 18:56 to 07:04 Oman — `/api/health` returns 503 `degraded`, and OPERATIONS.md §5d instructs the owner to "alert on non-200". The owner is paged in the middle of the night by a cold-start optimiser that failed once and will fix itself twelve hours later. Because line 84-85 of the health route  |
| P2 | heartbeat-dead-man | A bare authenticated `{"ok":true}` marks the nightly dump healthy, and the credential that does it is the one OPERATIONS tells the owner to paste into a third-party cron service | `route.ts:43` | `recordHeartbeat('db-backup', {ok:true,…})` writes a fresh `lastRunAt`, so `/api/health` reports `db-backup: ok` for the next 40 hours, every day, while the real nightly dump is broken — the precise failure B3 exists to catch (rotated `neondb_owner` password, expired R2 token, Neon host change), now maskable rather tha |
| P2 | importer-credit-guards | Credit limits are writable from any sheet without payment_terms changing, because temix_code equals cust_code for the whole migrated master | `imports.ts:1383` | The credit limit and payment-term days of any of the 3,252 CREDIT customers finance is supposed to be reviewing (runbook §0 item 5) can be set to any value up to 99,999,999,999 by whoever hands the Steward a spreadsheet. Payment terms never change, so nothing that watches for a CASH↔CREDIT flip sees anything, and the o |
| P2 | golive-passwords-and-bootstrap | mustChangePassword is enforced only by a path check in Edge middleware; no server action or route handler re-checks it, so an action can be dispatched from the one path the check allows | `auth.config.ts:146` | The middleware permits the request because it matches only on `pathname`, and nothing downstream looks at mustChangePassword: runAction (lib/errors.ts:114) does not, and every service guard (requireSteward at services/imports.ts:38, requireUserAdmin, etc.) checks role only. Someone who overheard a colleague's 8-digit s |
| P2 | audit-envelope | A regional Manager's /audit view silently drops every CustomerEdit, Attachment and Export row — including the three entity types its own filter offers | `page.tsx:76` | For a Manager the page never shows a single approval-chain row (STEP_APPROVE, APPROVE, REJECT, FINALIZE, ESCALATE are all entityType 'CustomerEdit'), never shows a photo detach ('Attachment'), and never shows an export of the customer master ('Export'). Picking one of those three options from the dropdown returns "No a |
| P2 | merge-and-duplicates | Re-parenting attachments made Attachment.customerId mutable; detachPhotoCore still reads it before its transaction, so a merge racing a CR-photo detach leaves the winner's crPhotoId pointing at a soft-deleted attachment | `duplicates.ts:358` | The surviving customer's record says it has a CR document and every consumer believes it: completenessScore keeps the +10 (lib/completeness.ts:46), the customer-master and change-report exports print `cr_photo: yes` (services/exports.ts:141, lib/change-report.ts:538), and the mandatory-document submit gate passes (serv |
| P2 | merge-and-duplicates | "Mark distinct" is a one-click, permanent, un-undoable suppression of a duplicate pair | `duplicates.ts:103` | findDuplicateCandidates treats the existence of ANY 'CustomerPair' row as "dismissed forever" in both directions (duplicates.ts:103-118), so the pair never appears in the queue again. There is no un-dismiss action anywhere in the app, no list of dismissed pairs to review, and the row cannot be removed: the auditlog_app |
| ~~P2~~ **FIXED** | app-role-grants | ~~restore-verify --expect-app-role documents that it checks the role's REVOKEs but only checks that the role name exists~~ — R-02 now asserts the privilege matrix with `has_table_privilege` (commit d30dffd) | `restore-verify.ts:487` | The script prints "✓ restore verified" and exits 0 in both failure directions. If grant was skipped, nmwc_app has no privileges at all on the restored database — pg_dump --no-privileges strips every GRANT — so pointing Vercel at the restored database gives "permission denied for table Customer" on literally every page, |
| P3 | heartbeat-dead-man | B5 turned the public /api/health into an unauthenticated, unrate-limited Postgres round-trip per request | `route.ts:47` | Each anonymous hit costs one Vercel function invocation plus one Neon round-trip, with `export const dynamic = 'force-dynamic'` (line 25) and no cache header, so nothing is ever served from an edge cache. Sustained traffic keeps the Neon compute endpoint permanently awake and consumes the project's finite compute-hour  |
| P3 | importer-credit-guards | The load fires one TEMIX_SYNC_ACKED notification per pre-existing customer, all to the same account | `imports.ts:1411` | Thousands of "Customer landed in Temix" notifications are written to one user's feed (the seeded customers all carry the same `createdById`, prisma/seed-muscat-customers.ts:208), burying the real approval and SLA notifications that the update flow depends on from day one. Each one is an extra write inside the per-group |
| ~~P3~~ **FIXED 2026-09-22** | golive-passwords-and-bootstrap | ~~account-master.xlsx holds every initial password in cleartext, but the runbook's clean-up step names only credentials.xlsx and managers.json~~ — the step now names all three files and says why the third is the one that gets forgotten. Verified against the built file: the Users sheet carries a live password on 51 of 62 rows | `GO-LIVE-RUNBOOK.md:121` | golive-data/account-master.xlsx stays on the laptop with a `password` column holding the live 8-digit secret for every salesman and for `accountant`, `finance.manager` and `gm.nmwc`. Those values are dead only for people who have actually signed in — and the runbook itself (§1 step 8) tells the Steward to check the nex |
| ~~P3~~ **FIXED 2026-09-20** | golive-passwords-and-bootstrap | ~~RECONCILIATION.md tells the owner Sara Khayat's second login is `sara.khayat.c3`; the builder actually issues `c3`~~ — the generated text now says `c3`, and the runbook's hand-out step was corrected with it | `build-masters.ts:1282` | There is no `sara.khayat.c3` row anywhere — the second, salesman login is `c3`. The runbook (§1 step 8) warns that a manager who also sells has two logins and that "they will try the wrong one"; the reconciliation document the owner is told to read after every build names a username that does not exist, which is exactl |
| P3 | audit-envelope | approveEditCore carries a comment stating create-finalize was never converted, contradicted by its own code 100 lines below | `edits.ts:860` | The comment says the two rows that prove a customer went live are still written directly and therefore carry null ip/userAgent. They are not — lib/create-finalize.ts:374 and :386 call writeAudit with the envelope handed down as a parameter. A maintainer who trusts it either re-does a conversion that already happened, o |
| P3 | audit-envelope | Export audit rows are stamped with the UTC calendar date, so an export taken between 00:00 and 03:59 Oman is filed under the previous day | `exports.ts:174` | The /audit page's only free-text filter is `entityId contains` (app/(app)/audit/page.tsx:45). An investigator asking "who exported the master on the 15th" and searching 2026-09-15 misses the 01:30 export entirely, and finds it filed alongside genuine 14 September exports. The off-by-one window — 00:00 to 03:59 local —  |
| P3 | audit-envelope | getAuditEnvelope caps userAgent at 500 chars but applies no cap to ip, from the same untrusted header set | `audit.ts:23` | The whole value lands verbatim in AuditLog.ip, which is Postgres `text` with no length or format check. The module header asserts the opposite invariant — that the row size is guarded — but the guard covers only userAgent. Because prisma/migrations/20260914150000_audit_immutability makes the table append-only (UPDATE,  |
| P3 | merge-and-duplicates | EXACT_TRIPLE duplicate detection keys on an arbitrary branch's region for any multi-branch customer, so real duplicates are missed non-deterministically | `duplicates.ts:156` | Postgres is free to return any one of that customer's live branches, and the choice can differ between page loads. Two genuine duplicates with identical legalName and primaryPhoneNorm both trading in region R are surfaced only if the arbitrary branch picked for each happens to be the one in R; otherwise the pair silent |
| P3 | merge-and-duplicates | A merge writes no audit row naming the customer it archived, so the loser's own forensic trail ends with no explanation | `duplicates.ts:440` | The /audit page filters and searches by entityId (app/(app)/audit/page.tsx:45, 77) and AuditLog is indexed on (entityType, entityId, at). Looking up the merged-away customer returns its CREATE and UPDATE history and then simply stops — nothing says it was archived, by whom, when, or into what. An operator asking "where |
| P3 | merge-and-duplicates | The duplicates empty state tells the Steward that phone and fuzzy-name matching came back clean, but both rules were deleted | `page.tsx:31` | The screen reads "No duplicates detected — Phone, CR, and fuzzy-name matches are all clean." The detector only ever runs exact CR-number matching and exact legalName + exact phone + same-region matching; PHONE-only and NAME-fuzzy were removed in P1.4 as deliberately as they are still advertised here. The Steward is giv |
| ~~P3~~ **FIXED** | audit-immutability | ~~`restore-verify --expect-app-role` claims to check the REVOKEs and only checks that a role named nmwc_app exists~~ — duplicate of the app-role-grants row above; both closed by the same change | `restore-verify.ts:487` | The operator's closing green says `R-01 least-privilege role nmwc_app exists — present ✓` and they conclude the restored database is back to the documented privilege posture. It is not: the role can hold every privilege on both ledgers, or none at all, and R-01 reports `pass` either way. The one check that would catch  |
